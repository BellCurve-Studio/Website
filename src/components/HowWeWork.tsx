...
export default function HowWeWork({ onOpenAudit }: HowWeWorkProps) {
  ...
  const steps = [
    {
      ...
      timeframe: "Day 1",
      description: "You tell us what's not working in your business or website. Zero high-pressure sales calls — just an open, honest conversation about friction points.",
      icon: Sparkles,
      accent: "#ffbd5f",
  ...
  // Continuous Next & Prev Step calculations (1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 1 -> 2...)
  ...
  // Auto-play effect with smooth continuous step cycling
  useEffect(() => {
  ...
  return (
    <section id="how-we-work" className="relative overflow-hidden border-b border-[#17232d]/15 bg-[#e7f0eb] py-16 lg:py-24 transition-colors duration-500">
      {/* Background Decorative Patterns */}
      <div className="absolute left-0 top-0 h-3 w-full bg-[#17232d]" aria-hidden="true" />
      <div className="absolute right-[-10rem] top-20 h-96 w-96 rounded-full border border-[#17232d]/10 pointer-events-none" aria-hidden="true" />
      <div className="absolute left-[-8rem] bottom-10 h-80 w-80 rounded-full border border-dashed border-[#17232d]/15 pointer-events-none" aria-hidden="true" />

      <div className="relative mx-auto max-w-7xl px-5 sm:px-7 lg:px-8 w-full">
        {/* Main Section Header */}
        <header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between scroll-reveal">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#17232d]/20 bg-[#fffdf8] px-3.5 py-1.5 font-code-mono text-[10px] font-black uppercase tracking-[.18em] text-[#56616a] shadow-[2px_2px_0_#17232d]">
              <Workflow className="h-3.5 w-3.5 text-[#ed542d]" aria-hidden="true" /> The 7-Step Framework
            </div>
            <h2 className="mt-4 text-4xl font-black leading-[.94] tracking-[-.07em] text-[#17232d] sm:text-5xl lg:text-6xl">
              How we <em className="font-editorial-serif font-normal text-[#ed542d]">work.</em>
            </h2>
          </div>

          <div className="flex flex-col gap-4 lg:max-w-md">
            <p className="border-l-2 border-[#ed542d] pl-4 text-sm leading-relaxed text-[#40505a] sm:text-base">
              A structured, transparent process designed to eliminate client risk, guarantee clarity, and deliver high-performance results.
            </p>

            {/* View Mode & Auto-Play Controls */}
            <div className="flex items-center gap-2">
              <div className="flex items-center rounded-xl border border-[#17232d] bg-[#fffdf8] p-1 shadow-[2px_2px_0_#17232d]">
                <button
                  onClick={() => setViewMode("stage")}
                  className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-all duration-300 ${
                    viewMode === "stage"
                      ? "bg-[#17232d] text-[#fffdf8] shadow-[1px_1px_0_#ed542d]"
                      : "text-[#56616a] hover:text-[#17232d]"
                  }`}
                >
                  <Sliders className="h-3.5 w-3.5" /> Stage View
                </button>
                <button
              ...
              {viewMode === "stage" && (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-xl border border-[#17232d] px-3.5 py-2 text-xs font-bold shadow-[2px_2px_0_#17232d] transition-all duration-300 hover:-translate-y-0.5 ${
                    isAutoPlaying
                      ? "bg-[#ffbd5f] text-[#17232d] shadow-[3px_3px_0_#ed542d]"
                      : "bg-[#fffdf8] text-[#17232d] hover:bg-[#f4efe5]"
                  }`}
                >
                  {isAutoPlaying ? (
                    <>
                      <Pause className="h-3.5 w-3.5 text-[#ed542d]" /> Pause Tour
                    </>
                  ) : (
                    <>
                      <Play className="h-3.5 w-3.5 text-[#ed542d]" /> Auto Tour
                    </>
                  )}
                </button>
              )}
        ...
        <div className="mt-10 rounded-2xl border-2 border-[#17232d] bg-[#fffdf8] p-4 shadow-[6px_6px_0_#17232d]">
          {/* TRACKER HEADING BAR */}
          <div className="flex items-center justify-between border-b-2 border-[#17232d]/10 pb-3 mb-3 px-1">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-[#ed542d] animate-pulse" />
              <h3 className="font-code-mono text-xs font-black uppercase tracking-[.18em] text-[#17232d]">
          ...
          <div className="overflow-x-auto pb-2 pt-1 scrollbar-none">
            <div className="flex min-w-[760px] items-center justify-between relative px-4 py-2">
              {/* Connected Track Line */}
              <div className="absolute left-8 right-8 top-1/2 -z-0 h-1.5 -translate-y-1/2 rounded-full bg-[#17232d]/10" />
              <div
                className="absolute left-8 top-1/2 -z-0 h-1.5 -translate-y-1/2 rounded-full bg-[#ed542d] transition-all duration-500 ease-out"
                style={{
              ...
              {steps.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`grid h-11 w-11 place-items-center rounded-xl border-2 transition-all duration-300 ${
                        isActive
                          ? "border-[#17232d] bg-[#ffbd5f] text-[#17232d] shadow-[4px_4px_0_#ed542d]"
                          : isPast
                          ? "border-[#17232d] bg-[#cbe9da] text-[#17232d] shadow-[2px_2px_0_#17232d]"
                          : "border-[#17232d]/30 bg-[#f4efe5] text-[#56616a] hover:border-[#17232d] hover:bg-[#fffdf8]"
                      }`}
                    >
                      {isPast ? (
                        <Check className="h-5 w-5 stroke-[3] text-[#17232d]" />
                      ) : (
                        <StepIcon className="h-4 w-4" />
                      )}
                    </div>
                    <span
                      className={`font-code-mono text-[10px] font-black uppercase tracking-wider whitespace-nowrap transition-colors duration-300 ${
        ...
        {/* VIEW MODE 1: LIGHT THEME INTERACTIVE STAGE */}
        {viewMode === "stage" && (
          <div
            key={animateKey}
            className="mt-6 rounded-[2rem] border-2 border-[#17232d] bg-[#fffdf8] p-6 sm:p-8 lg:p-10 shadow-[8px_8px_0_#17232d] transition-all duration-300 animate-tab-fade"
          >
            <div className="grid gap-8 lg:grid-cols-[1.1fr_1fr] lg:items-center">
              ...
              <div className="flex flex-col justify-between min-h-[340px]">
                <div>
                  <div className="flex items-center gap-3">
                    <span
                      className="grid h-11 w-11 place-items-center rounded-xl border-2 border-[#17232d] shadow-[3px_3px_0_#17232d] transition-transform duration-300 hover:rotate-3"
                      style={{ backgroundColor: activeStep.accent }}
                    >
                      <ActiveIcon className="h-5 w-5 text-[#17232d]" />
                    </span>
                    <div>
                  ...
                  <p className="mt-5 text-sm sm:text-base leading-relaxed text-[#40505a]">
                    {activeStep.description}
                  </p>

                  {/* Key Outcomes & Deliverables */}
                  <div className="mt-6 rounded-2xl border-2 border-[#17232d]/20 bg-[#f4efe5] p-5 shadow-[3px_3px_0_#17232d]/10">
                    <p className="font-code-mono text-[10px] font-black uppercase tracking-[.16em] text-[#56616a]">
                      Key Outcomes & Deliverables
                    </p>
                    <ul className="mt-3 grid gap-2.5 sm:grid-cols-1">
                      {activeStep.deliverables.map((item) => (
                        <li
                          key={item}
                          className="flex items-center gap-2.5 text-xs sm:text-sm font-bold text-[#17232d]"
                        >
                          <CheckCircle2 className="h-4 w-4 text-[#ed542d] shrink-0" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                ...
                {/* ENDLESS CONTINUOUS STEP NAVIGATION CONTROLS (1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 1 -> 2...) */}
                <div className="mt-8 flex items-center justify-between border-t-2 border-[#17232d]/10 pt-5">
                  <button
                    onClick={handlePrevStep}
                    className="inline-flex items-center gap-2 rounded-xl border-2 border-[#17232d] bg-[#f4efe5] px-5 py-2.5 text-xs font-black text-[#17232d] shadow-[3px_3px_0_#17232d] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#fffdf8]"
                  >
                    <ChevronLeft className="h-4 w-4 text-[#ed542d]" />
                    <span>Previous</span>
                  </button>

                  <div className="flex items-center gap-1.5 font-code-mono text-xs font-black text-[#56616a]">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border-2 border-[#17232d] bg-[#ed542d] px-5 py-2.5 text-xs font-black text-white shadow-[3px_3px_0_#17232d] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#d94b2c]"
                  >
                    <span>Next</span>
              ...
              {/* RIGHT PANE: 100% LIGHT THEME BESPOKE STEP SIMULATOR */}
              <div className="rounded-2xl border-2 border-[#17232d] bg-[#f8f4ec] p-5 sm:p-6 text-[#17232d] shadow-[6px_6px_0_#ed542d] min-h-[340px] flex flex-col justify-between relative overflow-hidden transition-all duration-500">
                        ...
                        <button
                          ...
                          className={`rounded-xl border-2 p-3 text-left transition-all duration-300 ${
                            selectedFriction === item.id
                              ? "border-[#17232d] bg-[#ffbd5f] text-[#17232d] shadow-[3px_3px_0_#17232d] -translate-y-0.5"
                              : "border-[#17232d]/20 bg-[#fffdf8] text-[#56616a] hover:border-[#17232d] hover:bg-white"
                          }`}
                        >
                          <span className="block text-xs font-black">{item.label}</span>
                          <span className="block text-[10px] font-bold text-[#ed542d] mt-0.5">{item.severity}</span>
                        </button>
                      ))}
                    </div>
                    <div className="rounded-xl border border-[#17232d]/20 bg-[#fffdf8] p-3.5 text-xs leading-relaxed text-[#17232d] shadow-[2px_2px_0_#17232d]/10">
                ...
                {activeStepIndex === 1 && (
                  <div className="my-4 space-y-4">
                    ...
                    <div className="space-y-3.5 rounded-xl border border-[#17232d]/20 bg-[#fffdf8] p-4 shadow-[3px_3px_0_#17232d]/10">
                      <div>
                  ...
                  <div className="my-4 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="rounded-lg border-2 border-[#17232d] bg-[#17232d] px-3 py-1.5 text-xs font-bold text-white shadow-[2px_2px_0_#ffbd5f] hover:bg-[#ed542d] transition-all"
                      >
                        Toggle: {showPlainLanguage ? "Plain English" : "Dev Jargon"}
                      </button>
                    </div>

                    <div className="rounded-xl border-2 border-[#17232d] bg-[#fffdf8] p-4 shadow-[4px_4px_0_#cbe9da] space-y-2">
                      <span className="font-code-mono text-[10px] uppercase font-black text-[#ed542d]">
                    ...
                    <div className="rounded-lg border border-[#17232d]/20 bg-[#cbe9da] p-2.5 text-[11px] font-black text-[#17232d]">
                ...
                {activeStepIndex === 3 && (
                  <div className="my-4 space-y-3">
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={`rounded-xl border-2 p-2.5 transition-all duration-300 ${
                            activeNode === node.id
                              ? "border-[#17232d] bg-[#e7d7ff] text-[#17232d] shadow-[3px_3px_0_#17232d] -translate-y-0.5"
                    ...
                    <div className="rounded-xl border-2 border-[#17232d] bg-[#fffdf8] p-3.5 text-xs font-bold leading-relaxed text-[#17232d] shadow-[2px_2px_0_#17232d]">
                ...
                {activeStepIndex === 4 && (
                  <div className="my-4 space-y-3">
                    <div className="rounded-xl border-2 border-[#17232d] bg-[#fffdf8] p-4 shadow-[4px_4px_0_#ffd7c5] space-y-2.5">
                      ...
                      <ul className="space-y-1.5 text-xs font-bold text-[#17232d]">
                        <li className="flex items-center gap-2"><Check className="h-4 w-4 text-[#ed542d]" /> Fixed Investment Scope (Zero Hidden Costs)</li>
                        <li className="flex items-center gap-2"><Check className="h-4 w-4 text-[#ed542d]" /> Guaranteed Delivery Timeline</li>
                        <li className="flex items-center gap-2"><Check className="h-4 w-4 text-[#ed542d]" /> Full Source Code & IP Ownership</li>
                      </ul>
                      <div className="mt-2 inline-block rounded border border-[#17232d] bg-[#ed542d] px-2.5 py-0.5 text-[9px] font-black text-white uppercase tracking-widest shadow-[1px_1px_0_#17232d]">
                ...
                {activeStepIndex === 5 && (
                  <div className="my-4 space-y-3 font-code-mono">
                    <div className="flex items-center gap-2 text-xs font-black text-[#17232d] border-b border-[#17232d]/15 pb-2">
                      <Terminal className="h-4 w-4 text-[#ed542d]" /> Founder Development Stream
                    </div>
                    <div className="space-y-1.5 rounded-xl border-2 border-[#17232d] bg-[#fffdf8] p-3.5 shadow-[3px_3px_0_#ffbd5f] text-xs font-bold">
                ...
                {activeStepIndex === 6 && (
                  <div className="my-4 space-y-4">
                    ...
                    <div className="space-y-2 rounded-xl border-2 border-[#17232d] bg-[#fffdf8] p-4 shadow-[3px_3px_0_#cbe9da]">
                      ...
                      <div className="h-3 w-full rounded-full bg-[#17232d]/10 overflow-hidden">
                ...
                <div className="mt-3 flex items-center justify-between border-t-2 border-[#17232d]/15 pt-3">
                  <span className="text-xs text-[#56616a] font-bold">
                    Need scoping for your project?
                  </span>
                  <button
                    onClick={onOpenAudit}
                    className="inline-flex items-center gap-1.5 rounded-lg bg-[#ed542d] px-3.5 py-1.5 text-xs font-black text-white shadow-[2px_2px_0_#17232d] hover:-translate-y-0.5 transition-all"
                  >
        ...
        {/* VIEW MODE 2: EXPANDED GRID BLUEPRINT MODE */}
        {viewMode === "grid" && (
          <div className="mt-6 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 animate-tab-fade">
            {steps.map((step, idx) => {
              const StepIcon = step.icon;
              return (
                <div
                  ...
                  className="group relative flex flex-col justify-between rounded-2xl border-2 border-[#17232d] bg-[#fffdf8] p-5 shadow-[4px_4px_0_#17232d] transition-all duration-300 hover:-translate-y-1 hover:shadow-[6px_6px_0_#ed542d] cursor-pointer"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <span
                        className="grid h-10 w-10 place-items-center rounded-xl border-2 border-[#17232d] shadow-[2px_2px_0_#17232d] transition-transform duration-300 group-hover:scale-110"
                        style={{ backgroundColor: step.accent }}
                      >
                        <StepIcon className="h-4 w-4 text-[#17232d]" />
                      </span>
                      <span className="font-code-mono text-xs font-black text-[#ed542d]">
                        {step.num}
                      </span>
                    </div>

                    <h3 className="mt-4 text-base font-black tracking-tight text-[#17232d]">
                      {step.title}
                    </h3>

                    <p className="mt-1.5 text-xs leading-relaxed text-[#56616a]">
                      {step.description}
                    </p>
                  </div>

                  <div className="mt-4 border-t-2 border-[#17232d]/10 pt-3 flex items-center justify-between">
                    <span className="font-code-mono text-[9px] font-black uppercase tracking-wider text-[#ed542d]">
                      {step.badge}
                    </span>
                    <span className="text-[10px] font-bold text-[#17232d] group-hover:underline flex items-center gap-0.5">
